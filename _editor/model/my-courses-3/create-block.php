 ...
 * @return string
 */
function my_courses_3_createblocks($page) {
    global $USER, $CFG;
    ...
    foreach ($courses as $course) {
        $backgroundimage = my_courses_3_couse_image(new core_course_list_element($course));
        $progress = my_courses_3_course_progress($course);
        $blocks .= "
            <div class=\"col-12 col-md-4 course-card\">
...
function my_courses_3_course_progress($course) {
    global $USER;
...
function my_courses_3_couse_image(core_course_list_element $course) {
    global $CFG, $OUTPUT;
    ...
    foreach ($course->get_course_overviewfiles() as $file) {
        ...
        if ($file->is_valid_image()) {
            ...
            return moodle_url::make_file_url(
                "{$CFG->wwwroot}/pluginfile.php", "/{$contextid}/{$component}/{$filearea}{$filepath}{$filename}",
            );
        }
    }
